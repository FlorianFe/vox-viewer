...
import { BufferAttribute, BufferGeometry, CubeUVReflectionMapping, LinearEncoding, LinearToneMapping, Mesh, NearestFilter, NoBlending, OrthographicCamera, PerspectiveCamera, RawShaderMaterial, RGBEEncoding, RGBEFormat, Scene, UnsignedByteType, Vector2, WebGLRenderTarget } from 'three';
import EnvironmentScene from './EnvironmentScene.js';
import { encodings, getDirectionChunk, texelIO } from '@google/model-viewer/lib/three-components/shader-chunk/common.glsl.js';
import { bilinearCubeUVChunk } from '@google/model-viewer/lib/three-components/shader-chunk/cube_uv_reflection_fragment.glsl.js';
const LOD_MIN = 4;
const LOD_MAX = 8;
...
export class PMREMGenerator {
    constructor(renderer) {
        ...
        this[_c] = [];
        this[_d] = [];
        this[_e] = new BlurMaterial(MAX_SAMPLES);
        this[_f] = new OrthographicCamera(0, 1, 0, 1, 0, 1);
        let lod = LOD_MAX;
        for (let i = 0; i < TOTAL_LODS; i++) {
            const sizeLod = Math.pow(2, lod);
            this[$sizeLod].push(sizeLod);
            let sigma = 1.0 / sizeLod;
            let roughness = (1 + Math.sqrt(1 + 4 * Math.PI * sizeLod)) / (2 * Math.PI * sizeLod);
            ...
            for (let face = 0; face < cubeFaces; face++) {
                ...
                position.set([].concat(...coordinates), positionSize * vertices * face);
    ...
    fromDefault() {
        ...
        const cubeUVRenderTarget = this[$sceneToCubeUV](defaultScene, DEFAULT_NEAR, DEFAULT_FAR);
     ...
     * origin).
     */
    fromScene(scene, near = DEFAULT_NEAR, far = DEFAULT_FAR) {
        const dpr = this.renderer.getPixelRatio();
        this.renderer.setPixelRatio(1);
    ...
    [(_a = $roughness, _b = $sigma, _c = $sizeLod, _d = $lodPlanes, _e = $blurMaterial, _f = $flatCamera, $sceneToCubeUV)](scene, near, far) {
        const params = {
        ...
        for (let i = 0; i < 6; i++) {
            ...
            this.renderer.setViewport(col * SIZE_MAX, i > 2 ? SIZE_MAX : 0, SIZE_MAX, SIZE_MAX);
    ...
    [$equirectangularToCubeUV](equirectangular) {
        const params = {
            magFilter: NearestFilter,
            minFilter: NearestFilter,
            generateMipmaps: false,
        ...
        uniforms.texelSize.value = new Vector2(1.0 / equirectangular.image.width, 1.0 / equirectangular.image.height);
    ...
    [$applyPMREM](cubeUVRenderTarget) {
        for (let i = 1; i < TOTAL_LODS; i++) {
            const sigma = Math.sqrt(this[$sigma][i] * this[$sigma][i] -
     ...
     */
    [$blur](cubeUVRenderTarget, lodIn, lodOut, sigma) {
        this[$halfBlur](cubeUVRenderTarget, this[$pingPongRenderTarget], lodIn, lodOut, sigma, 'latitudinal');
        this[$halfBlur](this[$pingPongRenderTarget], cubeUVRenderTarget, lodOut, lodOut, sigma, 'longitudinal');
    }
    [$halfBlur](targetIn, targetOut, lodIn, lodOut, sigmaRadians, direction) {
